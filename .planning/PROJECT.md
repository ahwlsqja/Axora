...
- [ ] Subaccount-first 실행 모드 — 첫 세션에서 Subaccount 생성 + 소액 위임, agent가 위임 범위 내에서 실행 (Direct Signing 제거)
...
**실행 모드:**
- Subaccount-first: 첫 세션에서 agent subaccount 생성 → 소액 위임 → agent가 위임 범위 내 실행
- Direct Signing 없음: 모든 실행은 delegated subaccount를 통해 수행
...
| Subaccount-first (Direct Signing 제거) | 핵심 차별점을 첫 세션에서 경험, 토스식 단순 UX, 단일 실행 모드로 복잡도 감소 | Decided 2026-03-15 |
| Bounded automation (완전 자율 운용 배제) | 사용자 통제 하의 실행만 허용, 신뢰와 안전성 우선 | — Pending |

---
*Last updated: 2026-03-15 after Subaccount-first architecture decision*
